...
public class LITPhostsTestRunner extends TorTestCaseHelper {
    
    Logger logger = Logger.getLogger(LITPhostsTestRunner.class);
    ...
    @TestId(id = "CXP9031127-1", title = "Upgrade ERIClitphosts rpm on the MS")
    @Test(groups={"ACCEPTANCE"})
    public void upgradeERIClitphostsRPM() throws TimeoutException, FileNotFoundException {

        rpmUpgradeOperator.initialise("hosts");
     ...
     * @PRIORITY HIGH
     */
    @TestId(id = "CXP9031127-2", title = "Run python test cases for ERIClitphosts package")
    @Test(groups={"CDB_REGRESSION", "ACCEPTANCE"})
    public void runERIClitphostsTests() {
     ...
     * @PRE Execution of test {@link #runERIClitphostsTests()}
     * @PRIORITY HIGH
     * @param className
     * @param name
     * @param failures
     */
    @TestId(id = "CXP9031127-3", title = "Parse xml outputs from python tests")
